...
# 5. 결과 저장 및 시각화
def save_clusters_and_visualizations(df, clustering_results, embeddings_2d, output_base='FINCH_result'):
...
def organize_images_by_clusters(result_folder, image_folder='unsplash_images', output_base_folder='FINCH_clustered_images'):
    ...
    print(" 모든 레벨을 반영해서 이미지 정리 완료")

# 메인 실행부
def main():
    # 파일 경로
    caption_file = 'output_explain.csv'
    image_folder = 'unsplash_images'
    ...
    # 5. 결과 저장 및 시각화
    save_clusters_and_visualizations(df, clustering_results, embeddings_2d)

    # 6. 이미지 계층 폴더 정리
    organize_images_by_clusters('FINCH_result', image_folder=image_folder, output_base_folder='FINCH_clustered_images')
